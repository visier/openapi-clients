...
import visier.model_query_apis.models
from visier.model_query_apis import rest
from visier.model_query_apis.api_response import ApiResponse, T as ApiResponseT
from visier.model_query_apis.configuration import Configuration
from visier.model_query_apis.exceptions import (
    ApiValueError,
    ApiException
)
...
USER_AGENT_VALUE = 'OpenAPI-python'
...
class ApiClient:
    ...
    def __init__(
        ...
        self._auth_code_flow_timeout = 1200
    ...
    def call_api(
        ...
        try:
            ...
            raise e
        # here will be added code to handle authorization error
    ...
    def __deserialize(self, data, klass):
        ...
        if isinstance(klass, str):
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                klass = getattr(visier.model_query_apis.models, klass)
    ...
    def _update_connection(self, force: bool = False):
        if self.configuration.client_id and self.configuration.client_secret:
            self._update_connection_oauth2()
        else:
            self._connect_basic()

    def _update_connection_oauth2(self):
        ...
        if self.configuration.refresh_token:
            self._refresh_token()
        elif self.configuration.username and self.configuration.password:
            self._connect_oauth_password()
        else:
            self._connect_auth_code()

    def _connect_oauth_password(self):
        ...
        self.configuration.access_token = self._request_token(body)
    ...
    def _connect_auth_code(self):
        ...
        url_prefix = self.configuration.host + "/v1/auth/oauth2"
        binding = CallbackBinding(self.configuration.redirect_uri)
        with CallbackServer(binding) as svr:
            query_args = {
                "apikey": self.configuration.api_key['ApiKeyAuth'],
                "response_type": "code",
                "client_id": self.configuration.client_id,
            ...
            if self.configuration.redirect_uri:
                query_args["redirect_uri"] = self.configuration.redirect_uri
    ...
    def _request_token(self, body: dict) -> str:
        url = self.configuration.host + "/v1/auth/oauth2/token"
        ...
        if self.configuration.redirect_uri:
            body["redirect_uri"] = self.configuration.redirect_uri
        auth = (self.configuration.client_id, quote(self.configuration.client_secret, safe=''))
        headers = {
            "apikey": self.configuration.api_key["ApiKeyAuth"],
        ...
        # TODO temporary using requests due to some problem with using rest_client
        response = requests.post(url=url,
                                 data=body,
                                 headers=headers,
                                 auth=auth)
        # auth_str = f"{self.configuration.client_id}:{quote(self.configuration.client_secret, safe='')}"
        ...
        self.configuration.refresh_token = response_json.get('refresh_token')
        return response.json()['access_token']

    def _connect_basic(self):
        url = urljoin(self.configuration.host, '/v1/admin/visierSecureToken')
        data = {
            'username': self.configuration.username,
            'password': self.configuration.password
        }
        if self.configuration.vanity:
            data['vanityName'] = self.configuration.vanity
        ...
        self.configuration.api_key['CookieAuth'] = f'VisierASIDToken={token}'
